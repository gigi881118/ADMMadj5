...
double genlasso_objective(const arma::mat &A,const arma::colvec &b, const arma::mat &D, const double lambda, const arma::colvec &x, const arma::colvec &z){
  arma::colvec eta = A*x;
  double lowerBnd = log(std::numeric_limits<double>::epsilon());
  double upperBnd = -lowerBnd;
  arma::colvec constrained_eta = arma::clamp(eta, lowerBnd, upperBnd);
  arma::colvec bhat(1.0 / (1.0 + arma::exp(-constrained_eta)));
  arma::colvec logloss = -(b % arma::log(bhat)+(1.0-b) % arma::log(1.0-bhat));
  //return(arma::mean(logloss) + lambda*norm(D*x,1));
  return(norm(logloss, 1) + lambda*norm(D*x,1));
}
...
arma::mat genlasso_factor(const arma::mat &A, double rho,const arma::mat &D){
  ...
  const int n = A.n_cols;
  arma::mat U;
  U = chol(A.t()*A + rho*D.t()*D); //+ rho*D.t()*D
  return(U);
}
// [[Rcpp::export]]
Rcpp::List admm_genlasso(const arma::mat& A, const arma::colvec& b, const arma::mat &D, const double lambda, const double reltol, const double abstol, const int maxiter, const double rho){
  ...
  double lowerBnd = log(std::numeric_limits<double>::epsilon());
  double upperBnd = -lowerBnd;
  arma::colvec mu; // Declare the variable 'mu'
  arma::colvec eta;
  arma::colvec deta;
  arma::colvec constrained_eta;
  arma::colvec sw;

  mu = (b+0.5)/2.0;
  eta = arma::log(mu / (1.0 - mu));

  constrained_eta = arma::clamp(eta, lowerBnd, upperBnd);
  mu = 1.0 / (1.0 + arma::exp(-constrained_eta));

  deta = 1.0 / (mu % (1.0 - mu));
  //sw = 1.0 / (arma::abs(deta) % (arma::sqrt(mu) % arma::sqrt(1.0 - mu)));
  sw =  arma::sqrt(mu) % arma::sqrt(1.0 - mu);
  ...
  // 2. set ready
  arma::colvec x(n,fill::randn); x/=100.0;
  arma::colvec z(D*x);
  arma::colvec u(D*x-z);
  arma::colvec q(n,fill::zeros);
  arma::colvec zold(z);
  arma::colvec x_hat(n,fill::zeros);
  //increase
  arma::colvec b0(eta + (b - mu)%deta);
  arma::colvec yw(b0 % sw);
  arma::mat sw_matrix = repmat(sw, 1, n);
  arma::mat Aw = A % sw_matrix;

  // 3. precompute static variables for x-update and factorization
  arma::mat Atb = Aw.t()*yw;
  //arma::mat Atb = A.t()*b0;
  arma::mat U = genlasso_factor(Aw,rho,D); // returns upper
  arma::mat L = U.t();
  ...
  for (k=0; k<maxiter; k++){
    ...
    //sw = 1.0 / (arma::abs(deta) % (arma::sqrt(mu) % arma::sqrt(1.0 - mu)));
    sw = arma::sqrt(mu) % arma::sqrt(1.0 - mu);
    sw_matrix = repmat(sw, 1, n);
    Aw = A % sw_matrix;
    yw = b0 % sw;
    Atb = Aw.t()*yw;
    U   = genlasso_factor(Aw,rho,D); // returns upper
    L   = U.t();
}
